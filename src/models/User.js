  ...
  try {
    ...
    if (validResponse(response)) {
      ...
      const updates = {
        ...(userOps.updateUser || {}),
        ...(userOps.addProducts
          ? { products: state.user.products.concat(userOps.addProducts) }
          : {}),
        ...(userOps.removeProducts
          ? {
              products: state.user.products.filter(
                id => !userOps.removeProducts.includes(id)
              )
            }
          : {}),
        ...(userOps.createUserProduct
          ? {
              userProducts: state.user.userProducts.concat([
                userOps.createUserProduct
              ])
            }
          : {}),
        ...(userOps.updateUserProduct
          ? {
              userProducts: mergeByKey("id", state.user.userProducts, [
                userOps.updateUserProduct
              ])
            }
          : {})
...
const preProcess = payload => ({
  ...payload,
  ...(payload.products
    ? { products: preProcessUserProducts(payload.products) }
    : {})
});

export const userReducer = createReducer(
  {
    [updateUserStoreAction]: (state, payload) => ({
      ...state,
      ...preProcess(payload)
    }),
